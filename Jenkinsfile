        	...
        	steps {
        		script {
        			timeout(5) {
		                ...
		                if (qg.status != 'OK') {
		                	rocketSend attachments: [[$class: 'MessageAttachment', color: 'red', text: "${qg.status}", title: 'Quality gate status']], channel: 'test-multiple-channels', failOnError: true, message: "Build ${currentBuild.displayName} for Job [${env.JOB_NAME} ${env.GIT_BRANCH}] analyzed. See ${env.BUILD_URL}. Build status: ${currentBuild.currentResult}", rawMessage: true
