        		...
        		script {
        			timeout(5) {
		                ...
		                if (qg.status != 'OK') {
							...
							sh """curl -X POST -H 'Content-Type: application/json' --data '{"text":"Project [${env.POM_ARTIFACTID} ${env.GIT_BRANCH}] analyzed. See . Quality gate status: ${qg.status}","attachments":[{"title":"Rocket.Chat","title_link":"https://rocket.chat","text":"Rocket.Chat, the best open source chat","image_url":"/images/integration-attachment-example.png","color":"#764FA5"}]}' http://10.130.214.117:3300/hooks/ZbS27zr2S27rZjdod/LAyWdR2hBXzwviBoD7c4sJKsZ4NP8bibmFMHkHaKPayjrAiz"""
		                }
		            }
        		}
        	}
